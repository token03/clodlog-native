...
export const SortHeader: React.FC<SortHeaderProps> = ({
  ...
  return (
    <XStack height={"$3"} backgroundColor={"$background"} paddingVertical={"$2"} gap={"$2"} justifyContent={"center"}>
      ...
      <SelectItem width={"40%"} size={"$2"} value={sort} onValueChange={onSortChange} items={SortOptions} label={"Sort By:"}/>
      <SelectItem width={"25%"} size={"$2"} value={sortDirection} onValueChange={onSortDirectionChange} items={SortDirectionOptions} label={"Direction:"} />
    </XStack>
  );
};