...
export const DisplayScreenCard = ({
  ...
  useEffect(() => {
    ...
    const getImageNumber = () => {
      ...
      const shouldPad =
        [...padSet].some((prefix) => card.set.id.startsWith(prefix)) &&
        card.set.id !== "cel25c";
      return shouldPad
        ? card.number.padStart(3, "0").toLowerCase()
  ...
  const isTrainerGallery = () => {
    return (
      !!card?.number.match(/^[tg]g/i) ||
      card?.rarity.toLowerCase() === "special illustration rare"
    );
  };
