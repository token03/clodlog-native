...
export const CardGrid = ({
  ...
  const [selectedCardsRecord, setSelectedCardsRecord] = React.useState<
    Record<string, Card[]>
  >({});
  ...
  useEffect(() => {
    const sortCards = (
      ...
      return [...data].sort((a: Card, b: Card) => {
        ...
        const compare =
          sortOrder === SortDirection.Asc ? compareAsc : compareDesc;
        ...
        switch (sortField) {
          ...
          case Sort.Rarity:
            return compare(
              mapRaritySortWeight(a.rarity),
              mapRaritySortWeight(b.rarity)
            );
          case Sort.Date:
            return compare(a.set.releaseDate, b.set.releaseDate);
          case Sort.Price:
            const aSortPrice = parseFloat(
              a.prices?.["normal"]?.["ungraded"]?.replace("$", "") || "0"
            );
            const bSortPrice = parseFloat(
              b.prices?.["normal"]?.["ungraded"]?.replace("$", "") || "0"
            );
          ...
          >
            ...
            {(sort === Sort.Price || settings.alwaysDisplayPrice) && (
              ...
              >
                <Paragraph size={"$1"}>
                  {convertCurrency(
                    item.prices?.["normal"]?.[settings.gridPriceType],
                    settings.currency
                  )}
                </Paragraph>
              </View>
  ...
  return (
    <FlatList
      ...
      columnWrapperStyle={
        settings.displayGridColumns > 1 ? { marginHorizontal: 13 } : null
      }
