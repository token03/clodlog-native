...
export function WishlistDialogButton({
    Icon,
    Name,
    Card,
    ...props
}: PopoverProps & { Icon?: any; Name?: string; Card?: Card }) {
    const [isEditing, setIsEditing] = useState<boolean>(false);
    const {
        items: wishlists,
        addCardToItem: addCardToWishlist,
        removeCardFromItem: removeCardFromWishlist,
    } = useWishlists();
    const router = useRouter();
    const isCardInWishlist = Object.values(wishlists).some(wishlist =>
        wishlist.cards.some(card => card.id === Card?.id)
    );
                        ...
                        {Object.values(wishlists).map(wishlist => (
                            <Fieldset key={wishlist.id} horizontal borderWidth={0} gap={'$3'}>
                                <Checkbox
                                    ...
                                    size={'$4'}
                                    checked={
                                        wishlist.cards.some(card => card.id === Card?.id) || false
                                    }
                                    onCheckedChange={checked =>
                                        handleCheckboxChange(wishlist.id, checked)
                                    }
                                >
                                    <Checkbox.Indicator>
                                        <Check />
