...
export function CollectionDialogButton({
    ...
    const {
        items: collections,
        addCardToItem: addCardToCollection,
        removeCardFromItem: removeCardFromCollection,
    } = useCollections();
    const router = useRouter();
    const isCardInCollection = Object.values(collections).some(collection =>
        collection.cards.some(card => card.id === Card?.id)
    );
                        ...
                        {Object.values(collections).map(collection => (
                            <Fieldset key={collection.id} horizontal borderWidth={0} gap={'$3'}>
                                <Checkbox
                                    ...
                                    size={'$4'}
                                    checked={
                                        collection.cards.some(card => card.id === Card?.id) || false
                                    }
                                    onCheckedChange={checked =>
                                        handleCheckboxChange(collection.id, checked)
                                    }
                                >
                                    <Checkbox.Indicator>
                                        <Check />
